...
import { disconnectGa4 } from "@/services/ga4";
import { useRouter } from "next/navigation";

export function Ga4ConnectButton({ label = "Forbind Google Analytics" }: { label?: string }) {
  return (
    <a
...
export function Ga4DisconnectButton() {
  ...
  return (
    <button
      ...
      disabled={isPending}
      className="flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-medium text-slate-500 hover:bg-slate-50 hover:text-red-600 transition-colors disabled:opacity-50"
    >
